...
class Blip2OPT(Blip2Base):
    ...
    def forward(self, samples):
        ...
        if 'qurey_noise' in samples.keys():
            query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1) + samples["qurey_noise"]
        else:
        ...
        # query_tokens [1,32,768] -> query_output.last_hidden_state [1,32,768] -> [1,32,2560]
        if 'qurey_noise' in samples.keys():
            inputs_opt = self.opt_proj(query_output.last_hidden_state + samples["qurey_noise"])
        else:
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            inputs_embeds = self.opt_model.get_input_embeddings()(opt_tokens.input_ids) # [1, 9] -> [1, 9, 2560]
            inputs_embeds = torch.cat([inputs_opt,inputs_embeds],dim=1)
    ...
    def extract_token(self, samples):
        image = samples["image"]
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image))
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(
