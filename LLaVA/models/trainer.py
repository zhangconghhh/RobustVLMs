...
class Trainer:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        """
        model.train()
        # altered by zc for LLava adversarial training
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        # keys: 'input_ids'=[16,473], 'labels'=[16,473], 'attention_mask'=[16,473], 'images'=[16,3,336,336]
        outputs = model(**inputs)
        # keys: 'loss', 'logits'=[16, 1048, 32000]

        # added by zc for adversrial training
        from .trainer_adv_llava import Qattack
        output_adv = Qattack(model, inputs)

        # Save past state if it exists
        # TODO: this needs to be fixed and made cleaner later.
        if self.args.past_index >= 0:  # -1
        ...
        if labels is not None:
            ...
            # altered by zc for adversrail training
            # print(outputs["loss"], 0.02*output_adv["loss"])
            loss = outputs["loss"]+0.02*output_adv["loss"] if isinstance(outputs, dict) else outputs[0]+0.02*output_adv[0]
